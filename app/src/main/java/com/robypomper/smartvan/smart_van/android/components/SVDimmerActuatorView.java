...
import com.robypomper.josp.jsl.android.components.JSLRangeActionView;
 ...
 * <p>
 * View's default layout {@link R.layout#def_view_sv_dimmer_actuator}
 */
public class SVDimmerActuatorView
        extends JSLRangeActionView
        implements SVBaseActuatorServiceView {

    // Constants

    private final static int LAYOUT = R.layout.def_view_sv_dimmer_actuator;
    ...
    public SVDimmerActuatorView(@NonNull Context context) {
        this(context, null);
    }

    public SVDimmerActuatorView(@NonNull Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SVDimmerActuatorView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        this(context, attrs, defStyleAttr, null, LAYOUT);
    }

    public SVDimmerActuatorView(@NonNull Context context, JSLRangeAction svService, @LayoutRes int layout) {
        this(context, null, svService, layout);
    }

    public SVDimmerActuatorView(@NonNull Context context, @Nullable AttributeSet attrs, JSLRangeAction svService, @LayoutRes int layout) {
        this(context, attrs, 0, svService, layout);
    }

    public SVDimmerActuatorView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr, JSLRangeAction svService, @LayoutRes int layout) {
        ...
        getActionHandler().setCmdDecreaseTxt(getResources().getString(R.string.lay_service_bottom_sheet_dimmer_actuator_decrease));
        getActionHandler().setCmdIncreaseTxt(getResources().getString(R.string.lay_service_bottom_sheet_dimmer_actuator_increase));
        getActionHandler().setCmdSetMinTxt(getResources().getString(R.string.lay_service_bottom_sheet_dimmer_actuator_set_min));
        getActionHandler().setCmdSetMaxTxt(getResources().getString(R.string.lay_service_bottom_sheet_dimmer_actuator_set_max));
    }
